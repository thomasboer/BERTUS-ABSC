...
# General variables.
tf.app.flags.DEFINE_integer('mask_threshold', 75, 'percentile of attention that are being masked')
tf.app.flags.DEFINE_string("source_domain", "restaurantDemo", "source domain (training set)")
tf.app.flags.DEFINE_string("target_domain", "bookDemo", "target domain (test set)")
tf.app.flags.DEFINE_integer("source_year", 2014, "year source data set")
tf.app.flags.DEFINE_integer("target_year", 2019, "year target data set")
tf.app.flags.DEFINE_integer("year", 2014, "year data set (for prepare_bert)")
...
tf.app.flags.DEFINE_string("train_path_target", "data/programGeneratedData/" + str(
    FLAGS.embedding_dim) + FLAGS.source_domain + "_train_" + str(FLAGS.year) + ".txt", "train data path")
...
tf.app.flags.DEFINE_string("source_path", "data/programGeneratedData/" + str(
    FLAGS.embedding_dim) + "_" + FLAGS.source_domain + "_" + str(FLAGS.source_year) + ".txt", "train data path")
tf.app.flags.DEFINE_string("target_path", "data/programGeneratedData/" + str(
    FLAGS.embedding_dim) + "_" + FLAGS.target_domain + "_" + str(FLAGS.target_year) + ".txt",
                           "formatted test data path")
...
tf.app.flags.DEFINE_string("source_embedding",
                           "data/programGeneratedData/" + FLAGS.embedding_type + "_" + FLAGS.source_domain + "_" + str(
                               FLAGS.source_year) + "_" + str(FLAGS.embedding_dim) + ".txt",
                           "source domain pre-trained BERT embeddings")
tf.app.flags.DEFINE_string("target_embedding",
                           "data/programGeneratedData/" + FLAGS.embedding_type + "_" + FLAGS.target_domain + "_" + str(
                               FLAGS.target_year) + "_" + str(FLAGS.embedding_dim) + ".txt",
                           "source domain pre-trained BERT embeddings")

# SVM train and test data.
# NOTE. Not used, kept from original code.
...
tf.app.flags.DEFINE_string("hyper_train_path_target",
                           "data/programGeneratedData/" + str(FLAGS.embedding_dim) + 'hypertraindata_target' + str(
                               FLAGS.year) + ".txt", "hyper train data path")
tf.app.flags.DEFINE_string("hyper_eval_path_target",
                           "data/programGeneratedData/" + str(FLAGS.embedding_dim) + 'hyperevaldata_target' + str(
                               FLAGS.year) + ".txt", "hyper eval data path")
...
# Test results.
tf.app.flags.DEFINE_string("results_file", "data/programGeneratedData/RESULTS/" + str(
    FLAGS.embedding_dim) + "results_" + FLAGS.source_domain + "_" + FLAGS.target_domain + "_" + str(
    FLAGS.year) + ".txt", "results for test")
tf.app.flags.DEFINE_string("results_file_nomask", "data/programGeneratedData/RESULTS/" + str(
    FLAGS.embedding_dim) + "results_" + FLAGS.source_domain + "_" + FLAGS.target_domain + "_" + str(
    FLAGS.year) + ".txt", "results for test")
tf.app.flags.DEFINE_integer("writable", 1, "one if writable")
tf.app.flags.DEFINE_integer("savable", 0, "one if savable")

tf.app.flags.DEFINE_integer("isTest", 0, "one if tests, 0 if training")
